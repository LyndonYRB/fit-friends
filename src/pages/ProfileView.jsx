...
export default function ProfileView() {
  ...
  // UI-only mock data (later replace with route state or backend fetch)
  const user = {
    name: "Alex",
    age: 28,
    ...
    bio:
      "Training for a 10K. I like early runs + weekend long runs. Down for mobility work after.",
    photos: [
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-[390px] flex-col">
        ...
        <main className="flex-1 overflow-y-auto px-6 pt-4 pb-28 space-y-5">
          ...
          <div>
            ...
            <p className="mt-1 text-slate-400">{user.subtitle}</p>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-gray-300">{user.bio}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 pt-2">
            ...
            <button
              type="button"
              onClick={() => alert("Block flow later")}
              className="flex h-12 items-center justify-center gap-2 rounded-full border border-white/10 bg-white/5 text-sm font-bold text-white hover:bg-white/10"
            >
        ...
        <footer className="sticky bottom-0 border-t border-white/10 bg-[#101c22]/80 p-4 backdrop-blur-sm">
          <button
            type="button"
            onClick={() => alert("Connect / match request later")}
